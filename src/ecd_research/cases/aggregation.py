...
from ecd_research.models import CaseRecord, DiseaseLabel, TherapyTiming
...
def aggregate_case_records(
    ...
    *,
    ...
    patient_total: int | None = 0 if validated else None
    ...
    field_coverage = {
        "disease_label": _count_field(validated, "disease_label"),
        "mutation": _count_field(validated, "mutation"),
        "therapies": sum(1 for r in validated if r.therapies),
        "symptoms_to_diagnosis": _count_field(validated, "symptoms_to_diagnosis"),
        "diagnosis_to_treatment": _count_field(validated, "diagnosis_to_treatment"),
    ...
    if not validated:
        ...
        if field_coverage["therapy_timing"] < n:
            gaps.append(
                ...
                f"of {n} records (abstract-limited)."
            )
        ...
        if field_coverage["symptoms_to_diagnosis"] < n:
            gaps.append(
                ...
                f"{n - field_coverage['symptoms_to_diagnosis']} of {n} records."
            )
        if field_coverage["diagnosis_to_treatment"] < n:
            gaps.append(
                f"Diagnosis-to-treatment interval not reported in "
                f"{n - field_coverage['diagnosis_to_treatment']} of {n} records."
            )
        if cns_unknown > 0:
            gaps.append(
                f"CNS involvement unclear or not reported in {cns_unknown} of {n} records."
            )
        gaps.append(
